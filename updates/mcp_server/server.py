...
def _get_splunk_client_for(app_id: str):
    """
    Look up app_id in apps.json and return a SplunkClient with per-app config.
    Falls back to the default env-var client if no per-app config exists.
    """
    try:
        ...
        reg    = importlib.import_module("app_registry")
        app_cfg = reg.get_app(app_id)
        if app_cfg:
            return splunk_mod.get_client_for_app(app_cfg)
    except Exception as exc:
        logger.warning("Could not load app registry for app_id=%s: %s", app_id, exc)
    return splunk_mod.get_client()
...
def _tool_fetch_logs_splunk(args: dict) -> dict:
    try:
        ...
        client     = _get_splunk_client_for(app_id)
