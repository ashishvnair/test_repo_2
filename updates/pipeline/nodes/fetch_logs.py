...
Fix: checks the MCP tool's returned 'status' field.
If the tool reports an error (e.g. Splunk unreachable), emits a proper
'fetch_logs' error SSE event instead of silently emitting 'done' with 0 lines.
"""
...
async def fetch_logs_node(state: dict) -> dict:
    ...
    events.append({
        "step":   "fetch_logs",
        "status": "running",
        "data":   f"Scanning logs in Splunk for {app_id}...",
    })
    ...
    try:
        ...
        tool_status     = str(logs_result.get("status", "ok"))

        # Check if the MCP tool itself reported a connection/query error
        if not entries and tool_status.startswith("error"):
            error_msg = tool_status  # e.g. "error: Connection refused"
            logger.error("fetch_logs_node: tool reported error: %s", error_msg)
            events.append({
                "step":   "fetch_logs",
                "status": "error",
                "data":   error_msg,
