...
async def stream_rca(
    ...
    Flow:
      1. Build initial RCAState
      2. Stream through connector.rca_graph (LangGraph astream)
         OR fall back to direct node calls if langgraph is unavailable
      3. After each node completes, drain its sse_events and yield them
      4. After all nodes, yield the "complete" event with the final result

    Parameters
    ----------
    app_id        : Application identifier (matches Splunk/Loki index label)
    since_seconds : Time window in seconds (e.g. 3600 = last 1 hour)
    source        : Log source — "splunk" or "loki"
    """
    import connector
    ...
    try:
        if connector.rca_graph is not None:
            ...
            # Track accumulated state manually — each astream() chunk contains only
            # the single node's output, not the full merged state.
            ...
            async for chunk in connector.rca_graph.astream(initial_state):
                ...
                # Check for early-exit conditions after key nodes
                ...
                # Similar reports found → surface panel, do NOT run LLM
                ...
                # Emit "complete" event after report_assembly
...
async def _run_nodes_direct(
    ...
    for node_name, node_fn, enabled in node_registry:
        ...
        # Clear previous sse_events so we only yield this node's events
        ...
        if node_name == "fetch_logs" and not current_state.get("entries"):
            yield _sse("complete", "done", {
                "results": [], "message": "No logs found in time window"
            })
            return

        if node_name == "clean_logs" and not current_state.get("all_incidents"):
            yield _sse("complete", "done", {
                "results": [], "message": "No error patterns found"
            })
            return
